...
import {
  ...
  ChevronRight, Loader2, AlertTriangle, Check, RotateCcw, Utensils
} from "lucide-react";
...
function computeGoals(p) {
  const kg = p.weightLb / 2.2046226;
  const cm = (Number(p.heightFt) * 12 + Number(p.heightIn)) * 2.54;
...
async function callClaude(system, user) {
  ...
  if (!res.ok) throw new Error("The estimator didn't respond (" + res.status + "). Try again.");
...
function Onboarding({ onDone }) {
  const [p, setP] = useState({
    age: "", sex: "female", heightFt: "5", heightIn: "6", weightLb: "",
    activity: "light", goal: "general", diet: "omnivore", notes: "",
  });
  const set = (k, v) => setP((s) => ({ ...s, [k]: v }));
  const ready = p.age && p.weightLb && p.heightFt;

  return (
    <div style={{ maxWidth: 560, margin: "0 auto", padding: "28px 18px 60px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
        <Salad size={22} color={C.beet} />
        <span style={{ fontFamily: FONT_MONO, fontSize: 12, letterSpacing: 1, color: C.muted, textTransform: "uppercase" }}>
          Set up your chart
        </span>
      </div>
      <h1 style={{ fontFamily: FONT_DISPLAY, fontSize: 30, lineHeight: 1.1, color: C.ink, margin: "6px 0 4px", fontWeight: 500 }}>
        Let&apos;s tune your daily targets
      </h1>
      <p style={{ color: C.muted, fontSize: 14.5, marginBottom: 22 }}>
        A few basics so your goals fit you. Everything stays approximate — this isn&apos;t medical advice.
      </p>
      ...
      <Field label="Weight">
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <input inputMode="numeric" value={p.weightLb} onChange={(e) => set("weightLb", e.target.value.replace(/\D/g, ""))}
            placeholder="150" style={{ ...inputStyle, width: 96 }} />
      ...
      <Field label="Activity level">
        <div style={{ display: "grid", gap: 8 }}>
          {ACTIVITY.map(([v, l, sub]) => (
            <button key={v} onClick={() => set("activity", v)} className="nc-row-pick"
              style={{ border: "1px solid " + (p.activity === v ? C.ink : C.line), background: p.activity === v ? C.greenSoft : "#fff" }}>
              <span style={{ fontWeight: 600, color: C.ink, fontSize: 14 }}>{l}</span>
              <span style={{ color: C.muted, fontSize: 12.5 }}>{sub}</span>
            </button>
          ))}
        </div>
      </Field>
      ...
      <button
        disabled={!ready}
        onClick={() => onDone({ ...p, allergies: p.notes })}
        className="nc-cta"
        style={{ background: ready ? C.beet : "#D9BEC7", cursor: ready ? "pointer" : "default", marginTop: 10 }}>
        Build my daily chart <ChevronRight size={18} />
      </button>
      {!ready && <p style={{ color: C.muted, fontSize: 12.5, marginTop: 8, textAlign: "center" }}>Add age, height and weight to continue.</p>}
    </div>
  );
}
...
export default function App() {
  ...
  if (!profile) {
    return (
      <div style={{ background: C.paper, minHeight: "100vh", fontFamily: FONT_UI }}>
        <StyleTag />
        <Onboarding onDone={(p) => { setProfile(p); setGoals(computeGoals(p)); }} />
      </div>
    );
  }
